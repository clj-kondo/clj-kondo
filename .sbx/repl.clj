...
(defn- start! [nm root port]
  (let [;; nrepl writes .nrepl-port itself but never cleans it up, so remove it
        ...
        cmd (format (str "cd %s && (setsid sh -c '"
                         ...
                         "-M:clj-kondo/dev:test:clear-main -m nrepl.cmdline "
                         "--bind 0.0.0.0 --port %s; rm -f %s/.nrepl-port"
                         "' > /tmp/nrepl-%s.log 2>&1 < /dev/null &)")
                    root port root (fs/file-name root))]
    (p/shell "sbx" "exec" nm "--" "sh" "-c" cmd)))

(defn -main [& args]
  (let [{:keys [root port]} (cli/parse-opts args {:spec cli-spec})
    ...
    (if-let [up (running-port root)]
      ...
      (let [port (or port (free-port))]
        ...
        (start! nm root port)
        (loop [n 0]
          (cond (listening? port) (do (println "ready") (println port))

...
For linter specific configuration, see the [linters](linters.md) page.
...
The `:level` maybe be set to `:error`, `:warning`, `:info` or `:off`.

### Namespace config

You can override linter config for just for one namespace using namespace metadata:

``` clojure
(ns foo
  {:clj-kondo/config '{:linters {:unresolved-symbol {:exclude [foo]}}}})

foo ;; no warning
```

### Ignore warnings in a sexpr

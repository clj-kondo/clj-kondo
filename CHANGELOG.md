...
- [#2900](https://github.com/clj-kondo/clj-kondo/issues/2900): `:discouraged-var`: new per-var `:position` option (`:call`, `:value`, or a set) to limit the warning to call position or value position. A var passed to a higher-order function such as `map` counts as `:value`.

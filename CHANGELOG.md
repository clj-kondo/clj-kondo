...
- macros from source: a `defmacro` tagged with `{:clj-kondo/macro true}` is automatically extracted into `.clj-kondo/` and registered as a `:macroexpand` hook on the next run. See [doc/hooks.md](doc/hooks.md#macros-from-source)

...
- Type checker: narrow the type of a local in the then-branch of `if` or the body of `when` when it is guarded by a known predicate, or an `or` of predicates. E.g. `(if (string? x) (inc x) ...)` will warn.

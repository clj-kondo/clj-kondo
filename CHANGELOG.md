...
- [#2807](https://github.com/clj-kondo/clj-kondo/issues/2807): NEW linter: `:conditional-build-up` which warns inside `let` when the same local map is rebound via successive `(if pred (assoc m ...) m)` patterns, suggesting `cond->` instead (default level: `:info`) ([@walber-araujo](https://github.com/walber-araujo))

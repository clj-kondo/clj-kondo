...
- NEW linter: `:misplaced-async-meta` which warns when `^:async` metadata is placed on a function's argument vector or on the whole `(fn ...)` form, where ClojureScript ignores it; it must go on the function name

...
(ns ^{:no-doc true} clj-kondo.impl.toolsreader.v1v2v2.clojure.tools.reader.impl.commons
  (:refer-clojure :exclude [char])
  (:require [clj-kondo.impl.toolsreader.v1v2v2.clojure.tools.reader.reader-types :refer [peek-char read-char]]
            [clj-kondo.impl.toolsreader.v1v2v2.clojure.tools.reader.impl.errors :refer [reader-error]]
            [clj-kondo.impl.toolsreader.v1v2v2.clojure.tools.reader.impl.utils :refer [numeric? newline? char]])
        ...
        (let [ns-idx (inc ns-idx)]
          (when-not (== ns-idx (count token))
            (let [sym (subs token ns-idx)]
              (when (and #_(not (numeric? (nth sym 0)))
                         (not (= "" sym))
                         (not (.endsWith ns ":"))
                         (or (= sym "/")
                             (== -1 (.indexOf sym "/"))))

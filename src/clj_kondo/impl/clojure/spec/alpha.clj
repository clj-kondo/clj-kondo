...
(ns clj-kondo.impl.clojure.spec.alpha
...
(defn spec?
  ...
  [x]
  (when (instance? clj_kondo.impl.clojure.spec.alpha.Spec x)
    x))
...
#_(defmacro fdef
  ...
  (s/fdef clojure.core/symbol
    ...
    :ret symbol?)"
  [fn-sym & specs]
  `(clj-kondo.impl.clojure.spec.alpha/def ~fn-sym (clj-kondo.impl.clojure.spec.alpha/fspec ~@specs)))
  ...
  (let [{:keys [::op ps ks forms splice p1 rep+ maybe amp] :as p} (reg-resolve! p)]
    ...
    (when p
      (case op
            ::accept nil
            nil p
            ::amp (list* 'clj-kondo.impl.clojure.spec.alpha/& amp forms)
            ::pcat (if rep+
                     (list `+ rep+)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;; non-primitives ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(clj-kondo.impl.clojure.spec.alpha/def ::kvs->map (conformer #(zipmap (map ::k %) (map ::v %)) #(map (fn [[k v]] {::k k ::v v}) %)))
...
(defmacro keys*
  ...
  `(let [mspec# (keys ~@kspecs)]
     (clj-kondo.impl.clojure.spec.alpha/& (* (cat ::k keyword? ::v any?)) ::kvs->map mspec#)))

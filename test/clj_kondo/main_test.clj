...
(deftest conflicting-aliases-test
  ...
  (is (empty? (lint! "(ns foo (:require [foo.bar :as foo-bar] [foo.foo :as foo]))"
                     {:linters {:conflicting-alias {:level :error}
                                :unused-namespace {:level :off}}})))
  (testing "an alias equal to the current ns name resolves like any alias, issue #2929"
    (is (empty? (lint! "(ns json (:require [cheshire.core :as json]))
(defn parse-string [s] (json/parse-string s))"
                       {:linters {:conflicting-alias {:level :error}
                                  :unused-namespace {:level :off}}})))))

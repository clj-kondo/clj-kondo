...
(deftest resolve-name-test
  (let [ctx {:used-namespaces (atom {})
    ...
    (assert-submap
     '{:ns bar :name quux}
     (resolve-name ctx false 'foo 'quux))
...
"))]
      (is (= '{:ns rewrite-clj.parser :name parse-string :alias p}
             (resolve-name ctx false 'clj-kondo.impl.utils 'p/parse-string))))
...
"))]
        (is (= 'p (:unresolved-ns (resolve-name ctx false 'clj-kondo.impl.utils 'p/parse-string))))))
...
"))]
        (is (=
             ...
             (resolve-name ctx false 'clj-kondo.impl.utils 'clojure.core/inc)))))))

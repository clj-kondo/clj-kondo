...
(deftest analyze-ns-test
  (is
   (= (vars/analyze-ns-decl (parse-string "(ns foo (:require [bar :as baz :refer [quux]]))"))
      '{:type :ns, :name foo,
...
")))))
  (testing "calling functions from file without ns form"
    (is (submap?
         '{:calls {user [{:type :call, :name foo, :qname user/foo, :arity 1, :row 2, :col 16, :ns user, :filename "<stdin>", :lang :clj}]}, :defns {user #:user{foo {:type :defn, :name foo, :qname user/foo, :fixed-arities #{1}, :ns user, :filename "<stdin>", :lang :clj}}}}
         (vars/analyze-arities "<stdin>" :clj
                               (parse-string-all "
(defn foo [x]) (foo 1)
"))))))
